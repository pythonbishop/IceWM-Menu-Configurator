...
class Application:
    ...
    def move_item_up(self, event):
        ...
        parent = self.menu_tree.parent(iid)
        new_index = self.menu_tree.index(iid) + 1
        self.menu_tree.move(iid, parent, new_index)
        self.menu_tree.focus(iid)
    ...
    def saveas_dialog(self):
        try:
            ...
            with open(filepath, "w") as f:
                message = "# this file was autogenerated by the IceWM Menu Configurator\n\n"
                f.write(message+self.output)
            self.save_success()
        except:
            self.save_failed()
